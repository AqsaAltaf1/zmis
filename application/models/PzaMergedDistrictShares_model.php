<?php
class PzaMergedDistrictShares_model extends CI_model{
	...
	function get_district_payment_record()
	{
		$this->db->select("*");
	...
	function getMergedDistrictShares()
	{
		$this->db->select("*");
		$this->db->from("district_shares");
		$this->db->where('districtType','Merged');
		//$this->db->limit(5);
		$this->db->order_by('id',"ASC");
		return $users = $this->db->get()->result_array();
	}
	...
	function getSettledDistrictShares()
	{
		$this->db->select("*");
		$this->db->from("district_shares");
		$this->db->where('districtType','Settled');
		//$this->db->limit(5);
		$this->db->order_by('id',"ASC");
		return $users = $this->db->get()->result_array();
	}
