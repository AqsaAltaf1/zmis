...
class Dist_login_model extends CI_model 
{
	...
	function chk_account($username,$password)
	{
		...
		$district_password = $get_dist_qry->row('password');
			...
			{
			$this->session->set_flashdata('success','Please Update Your Password Before Further Processing on ZMIS...!');
		...
		function get_all_lzc($userid,$districtid)
		{
			//$str = "lzcname."(".id.")";
			$this->db->select("id,lzc_name"); //CONCAT(lzc_name,'(',id,')') as lzc_name"
		...
		function get_all_lzc_GA($userid)
		{
			//$str = "lzcname."(".id.")";
			$this->db->select("LZC_ID as id,LZC_Name as lzc_name"); 
			$this->db->from("zakatentryforms");
			$this->db->where('GS_Username',$userid);
			// $this->db->limit(2);
			$this->db->order_by('LZC_ID',"ASC");
			return $users = $this->db->get()->result_array();
		}
		
		function get_all_lzc_LZ11($userid)
		{
			//$str = "lzcname."(".id.")";
			$this->db->select("LZC_ID as id,LZC_Name as lzc_name"); 
			$this->db->from("zakatentryforms");
		...
		function get_all_lzc_LZ19($userid)
		{
			//$str = "lzcname."(".id.")";
			$this->db->select("LZC_ID as id,LZC_Name as lzc_name"); 
			$this->db->from("zakatentryforms");
		...
		{
			//$str = "lzcname."(".id.")";
			$this->db->select("lzc_name,0 as TargetNOBs, count(*) as RegNOBs, 0 as RemNOBs ");
			$this->db->from("lzc_list");
			$this->db->where('gs_username',$username);
			$this->db->group_by('lzc_name'); 
			//$this->db->limit(5);
			$this->db->order_by('lzc_name',"ASC");
			return $users = $this->db->get()->result_array();
		}

	function get_lzc_data($lzcname)   // For LZ-19
		{
			//$str = "lzcname."(".id.")";
	...
	function get_lzc_selected_data($lzcname)   // For LZ-11
		{
			//$str = "lzcname."(".id.")";
			$this->db->select("mustahiq_name,father_name,mustahiq_cnic,contact_number,gender ");
			$this->db->from("mustahiqeen");
	...
	function get_DD_Values($category)   // Dropdowns
		{
			//$str = "lzcname."(".id.")";
			$this->db->select("Concat(field_name,field_name_ur) as field_name");
