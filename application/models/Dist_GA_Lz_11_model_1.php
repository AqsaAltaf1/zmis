...
class Dist_GA_Lz_11_model extends CI_model{
	...
	{
	//"mustahiqeen.status=1 and" Ya remove kia ha, query sy, coz problem faced by Audit officer during rejection
	 //$query = $this->db->query("SELECT LZC_name as id,LZCActualName, count(*) as NOB FROM guzara_allowance_mustahiqeen where district_id= '".$userid."' group by LZC_name,LZCActualName ");
	   $query = $this->db->query("select lzc_list.id ,lzc_list.lzc_name as LZCActualName, count(mustahiqeen.LZC_id) as NOB from lzc_list LEFT JOIN mustahiqeen ON mustahiqeen.LZC_id=lzc_list.id where  lzc_list.district_id= '".$userid."' and mustahiqeen.year= '".$getfinancial_year."' GROUP by lzc_list.lzc_name, lzc_list.id");
