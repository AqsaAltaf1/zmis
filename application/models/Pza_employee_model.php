<?php
class Pza_employee_model extends CI_model{

	
	function manage_pza_employees($formArray)
	{
		$this->db->insert("employees",$formArray);
		}
		
	function updateAddPost ($distPosting,$formArray,$post_name)
	
	{
		$this->db->where('district_id',$distPosting);
		$this->db->where('post_name',$post_name);
		$this->db->set('no_of_posts', 'no_of_posts - 1', FALSE);
		$this->db->update('add_post',$formArray);
	}


	function updateHqDesig ($hqPosting,$formArray,$hqDesignation)
	
	{
		$this->db->where('district_id',$hqPosting);
		$this->db->where('post_name',$hqDesignation);
		$this->db->set('no_of_posts', 'no_of_posts - 1', FALSE);
		$this->db->update('add_post',$formArray);
	}
	...
	function get_designation_list()
	{
		...
		$this->db->where('post_location','District Level');
		$this->db->where('no_of_posts!=',0);
		//$this->db->limit(5);
		$this->db->order_by('id',"ASC");
		return $designation_list = $this->db->get()->result_array();
	}
	
	function get_pzaDesignation_list()
	{
		$this->db->select("*");
		$this->db->from("add_post");
		$this->db->where('post_location','Head Quarter Level');
		$this->db->where('no_of_posts!=',0);
		//$this->db->limit(5);
		$this->db->order_by('id',"ASC");
		return $designation_list = $this->db->get()->result_array();
	...
	function get_pza_employee_list()
	{
		$this->db->select("*");
		$this->db->from("employees");
		//$this->db->limit(5);
		$this->db->order_by('id',"DESC");
		return $employee_list = $this->db->get()->result_array();
	}
	...
	function get_zakat_staff_listing()
	{
		$this->db->select("*");
		$this->db->from("zakat_paid_staff_list");
		//$this->db->limit(5);
