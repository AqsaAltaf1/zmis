<?php
class Dist_HC_mustahiq_reg_model extends CI_model{

	
	function dist_manage_mustahiq_register($formArray)
	{
		$this->db->insert("hc_mustahiqeen",$formArray);
	}
	
	
	
	function get_all_hostpital_list()
	{
		$this->db->select("*");
	...
	function get_HC_mustahiq_list($userid,$getfinancial_year,$getfinancial_installment)
	{
		$this->db->select("*");
...
function getHealthMustahiqPrint($userid,$getfinancial_year,$getfinancial_installment)
	{
		$this->db->select("*");
		$this->db->from("hc_mustahiqeen");
		$this->db->where("district_id",$userid);
	...
	function DeleteRecord($id){
		$query="DELETE FROM `hc_mustahiqeen` WHERE id=$id";
		$this->db->query($query);
	...
	function updatePatientRecord($formArray,$userid, $getrecord_id)
		{
		$this->db->where('id',$userid);
		$this->db->where('district_id',$getrecord_id);
		$this->db->update('hc_mustahiqeen',$formArray);		
		}
		
	function patientInfo($patientID)
		{
			return $this->db->select('*')->from('hc_mustahiqeen')
			->where('hc_mustahiqeen.id', $patientID)->get()->result();			
		}
