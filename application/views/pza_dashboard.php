...
<div class="info-box-content text_align">
<span class="info-box-number">PZA Allocation</span>
<span class="info-box-number" style="color: green;"> 

<?php
$runpzf_query_amount = $this->db->select_sum('pza_amount')->from('pza_fund')->where('status',1)->where('financial_year',$getfinancial_year)->where('installment',$getfinancial_installment)->get();
//$this->db->last_query();
...
$y = $total_pzareceived_amount * 100;
$z = $y/$x;
echo round($z)."%";
?>
</small>
...
<?php
$net_balance =  $total_pzfreceived_amount - $total_pzareceived_amount;
...
<div class="info-box-content text_align">
<span class="info-box-number" >PZA Releases</span>
<span class="info-box-number" style="color: green;">
...
<div class="card card-primary collapsed-card">
<div class="card-header">
<h3 class="card-title">District/Hospital/Head Wise Search Form</h3>
...
<option value="">--- Select Hospital / District ----</option>
<?php
$i=1;
if(!empty($get_all_districts))
{
...
<a target="_blank" href="<?php echo base_url(); ?>Pza_dashboard/pza_printlist" class="btn btn-sm btn-primary float-right">Print List</a>
</div>
<!-- /.card-header -->
<div class="card-body">
...
<td> 
<?php
$select_MA_unspent = $this->db->select('*')->from('pzf_fund')->where('district_hospital_id',$getdistricts['id'])->where('account_head','Marriage Assistance')->where('financial_year',$getfinancial_year)->get();
$getMA_unspent= $select_MA_unspent->row('payment_received');
echo number_format($select_MA_unspent->row('payment_received'),1);
?>
</td>
...
<?php
$select_EST_unspent = $this->db->select('*')->from('pzf_fund')->where('district_hospital_id',$getdistricts['id'])->where('account_head','Educational Stipends (Technical)')->where('financial_year',$getfinancial_year)->get();
$getEST_unspent= $select_EST_unspent->row('payment_received');
echo number_format($select_EST_unspent->row('payment_received'),1);
//echo $this->db->last_query();
?>
...
<?php
$select_AE_unspent = $this->db->select('*')->from('pzf_fund')->where('district_hospital_id',$getdistricts['id'])->where('account_head','Administrative Expenditure')->where('financial_year',$getfinancial_year)->get();
$getAE_unspent= $select_AE_unspent->row('payment_received');
echo number_format($select_AE_unspent->row('payment_received'),1);
//echo $this->db->last_query();
?>
...
<?php
$total_unspent = $getGA_unspent + $getMA_unspent + $getDM_unspent + $getESA_unspent + $getESP_unspent + $getHC_unspent + $getEST_unspent + $getLZF_unspent + $getDZF_unspent + $getAE_unspent + $getLZC_chr_unspent;
