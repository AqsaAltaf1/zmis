...
$gsuserid = $this->session->userdata('gs_id');
$get_gsuserid = $this->db->select('*')->from('lzc_list')->where('gs_id',$gsuserid)->get();
...
$total_rzf_disbursement = $total_ga_amount + $total_ma_amount + $total_dm_amount + $total_hc_amount + $total_esa_amount + $total_esp_amount;
...
// Regular zakat Fund disbursement
...
<div class="row mb-2">
<div class="col-md-12">
<h3 class="m-0 text-dark">Welcome to Group Secretary <strong><?php echo $entityName;?></strong> <strong> (<?php echo $username;?>)</strong> District <strong><?php echo $district_name; ?></strong> Dashboard</h3></div>
<!-- /.col -->
...
<?php
$querycountLZC = $this->db->query('SELECT * FROM `lzc_list` WHERE gs_username = "'.$username.'" AND status = 1');
...
<?php
// Display the count of gs_selected_lzcs passed from the controller
// echo  $count_gs_selected_lzcs;
...
<?php 
// $queryZakatForm = $this->db->query('SELECT * FROM `zakatentryforms` WHERE GS_Username = "'.$username.'" AND status = 1');
// $countqueryZakatForm =  $queryZakatForm->num_rows();

// echo $countqueryZakatForm; 
$countqueryZakatForm = count($getFormReceivedLZC);
echo $countqueryZakatForm;
?>
