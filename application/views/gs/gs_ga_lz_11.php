...
$CategoryQuery = $this->db->query("SELECT category, COUNT(*)as mustaghiqCategory FROM `mustahiqeen` WHERE `year` = '".$getfinancial_year."' AND `Remarks` != 'Reject' AND approved_status = 1 AND payment_status = 1 AND `district_id` = '".$userid."' AND `addedby` = '".$username."'  GROUP BY category order BY category ");
...
<div class="info-box-content text_align">
<span class="info-box-number"><?php echo $category[0]['category'];?>(Approved)</span>
<span class="info-box-number" style="color: green;"> 
<?php
...
<?php
$i=1;
if(!empty($getGsLzclist))
{
foreach($getGsLzclist as $lzclist)
{
?>
