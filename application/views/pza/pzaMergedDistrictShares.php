...
$RZF_nf = number_format($totalRZFMerged,2);
...
<div class="row mb-2">
<div class="col-sm-8">
<h3 class="m-0 text-dark">Merged District Share in Zakat Fund Based on Population  </h3>
</div>
...
$GA_68 = 167484000;
echo $GA_nf = number_format($GA_68,2); 
...
$MA_12 = 29550000;
echo $MA_nf = number_format($MA_12,2); 
?>
...
$DM_08 = 19704700;
echo $DM_nf = number_format($DM_08,2); 
?>
...
$ES_08 = 19704700;
echo $ES_nf = number_format($ES_08,2); 
?>
...
<h3 class="card-title">Merged District Share in Zakat Fund  Based on Population (Provincial Level) For Current Financial Year <strong><?php echo $getfinancial_year;?></strong> </h3>

<a target="_blank" href="<?php echo base_url(); ?>PzaMergedDistrictShares/PrintPzaMergedDistrictShares" class="btn btn-sm btn-primary float-right">Print District-Wise RZH Shares</a>
</div>
<!-- /.card-header -->
...
<?php
$i=1;
if(!empty($getMergedDistrictShares))
{
foreach($getMergedDistrictShares as $mergedDistrictShares)
{
?>
<tr>
<td><?php echo $i;?></td>
<td><?php echo $mergedDistrictShares['district_name'].'<br>';
echo $population = $mergedDistrictShares['population']; ?></td>
<td><?php echo $populationRatio = $mergedDistrictShares['populationRatio']; ?></td>
<td> 
<?php  
/*$population_percent = $population * 100;
$dist_ratio = $population_percent/$total_population;
echo round($dist_ratio,1)."%";*/
echo $MergedGuzaraNOB = $mergedDistrictShares['NOB']; 
?>
</td>
<td><?php echo $MergedMarriageNOBs =  $mergedDistrictShares['marriageNOB'];?> </td>
<td>
<?php 
$mergedDistrictGAShare = $MergedGuzaraNOB * 12000;
echo number_format($mergedDistrictGAShare);
?> 
</td>
<td>
<?php 
$mergedDistrictMAShare = $MergedMarriageNOBs * 30000;
echo number_format($mergedDistrictMAShare);
?>  
</td>
<td>
<?php 
$mergedDMAmount = 19704737;
$mergedDistrictDMRatio = ($mergedDMAmount * $populationRatio) /100;
$mergedDistrictDMShare = floor((($mergedDistrictDMRatio)/100))*100;
echo $mergedDistrictDMShare_nf =number_format($mergedDistrictDMShare);

?> 
</td>
<td>
<?php 
$mergedESAmount = 19704737;
$mergedDistrictESRatio = ($mergedESAmount * $populationRatio) /100;
$mergedDistrictESShare = floor((($mergedDistrictESRatio)/100))*100;
echo $mergedDistrictESShare_nf =number_format($mergedDistrictESShare);
?> 
</td>

<td>
<?php 
$mergedHCAmount = 9852369;
$mergedDistrictHCRatio = ($mergedHCAmount * $populationRatio) /100;
$mergedDistrictHCShare = floor((($mergedDistrictHCRatio)/1000))*1000;
echo $mergedDistrictHCShare_nf =number_format($mergedDistrictHCShare);
?> 
</td>
<td>

<?php 
$districtWiseMergedFund = $mergedDistrictGAShare + $mergedDistrictMAShare + $mergedDistrictDMShare +$mergedDistrictESShare + $mergedDistrictHCShare; 
echo $total_fund_nf = number_format($districtWiseMergedFund);
?> 
...
$this->db->where("district_id",$mergedDistrictShares['id']);
...
<form action="<?php echo base_url(); ?>PzaMergedDistrictShares/manage_district_payment" method="post" enctype="multipart/form-data" onsubmit="return validateForm(<?php echo $i;?>)" name="district_allocation">
...
<option value="<?php echo $mergedDistrictShares['id'];?>"><?php echo $mergedDistrictShares['district_name'];?></option>
</select>
</div>
...
<input type="number" name="GA" id="ga" value="<?php echo round($mergedDistrictGAShare);?>" required class="form-control col-md-8 col-xs-12" readonly>
</div>

<div class="row form-group">
<label class="control-label col-md-3" for="amount_alloc">Payment for Marriage Assistance <span class="required">*</span> </label>
<input type="number" name="MA" id="ma" value="<?php echo round($mergedDistrictMAShare);?>" required class="form-control col-md-8 col-xs-12" readonly>
</div>

<div class="row form-group">
<label class="control-label col-md-3" for="amount_alloc">Deeni Madaris <span class="required">*</span> </label>
<input type="number" name="DM" id="dm" value="<?php echo round($mergedDistrictDMShare);?>" required class="form-control col-md-8 col-xs-12" readonly> 
</div>
<div class="row form-group">
<label class="control-label col-md-3" for="amount_alloc">Health Care <span class="required">*</span> </label>

<input type="number" name="HC" id="hc" value="<?php echo round($mergedDistrictHCShare);?>" required class="form-control col-md-8 col-xs-12"  readonly>
</div>
<div class="row form-group">
<label class="control-label col-md-3" for="amount_alloc">Educational Stipend (Academic) <span class="required">*</span> </label>

<input type="number" name="ESA" id="esa" value="<?php echo round($mergedDistrictESShare);?>" required class="form-control col-md-8 col-xs-12" readonly>
</div>
<div class="row form-group">
