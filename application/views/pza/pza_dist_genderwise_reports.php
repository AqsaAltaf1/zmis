...
<div class="col-md-3">
<form action="<?php echo base_url(); ?>Pza_dist_genderwise_reports/graphdata" method="post">
<select class="form-control" id="yearWiseReportGraph" name="yearWiseReportGraph">

<?php
$this->db->select("*");
$this->db->group_by("financial_Year");
$this->db->order_by('id',"DESC");
$query = $this->db->get('year_installment');
$query = $query->result();

foreach ($query as $getYear) 
{
 ?> 
 <option value="<?php echo $getYear->financial_Year;?>" > <?php echo $getYear->financial_Year;?></option>
 <?php
}
?>
</select>
...
<div class="col-md-5">
<a href="<?php echo base_url(); ?>Pza_total_mustahiqeen_report" target="_blank" style="margin-left:32%;" class="btn btn-success">Overall Mustahiqeen Report</a>
