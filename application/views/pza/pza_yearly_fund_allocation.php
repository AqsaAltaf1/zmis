...
$totalPzfReceived = $this->db->select_sum('payment_received')->from('pzf_fund')->where('status',1)->get();
$total_pzfreceived_amount = $totalPzfReceived->row('payment_received');

$totalPzfReleases = $this->db->select_sum('yearly_fund')->from('pza_yearly_fund_allocation')->where('status',1)->get();
$PzfReleases = $totalPzfReleases->row('yearly_fund');


$netPzfBalance =  $total_pzfreceived_amount - $PzfReleases;
...
<h5 class="m-0 text-dark" class="pull-right"> Financial Year: <b> <?php echo $getfinancial_year;?></b> </h5>
...
<span class="info-box-number">Fund Available at PZF A/C # 03</span>
<span class="info-box-number" style="color: blue;">
<?php

echo $net_balance_pzf= number_format($netPzfBalance,2);
?>
  <br>
</span>
...
/*$pzfBalanceAfterRelease =  $netPzfBalance - $total_reserved_amount;
echo $pzfBalanceAfterRelease_nf= number_format($pzfBalanceAfterRelease,2);*/
?>
...
<input type="number" id="yearly_fund" min="1" max="<?php echo $netPzfBalance;?>" name="yearly_fund" required data-validate-minmax="" class="form-control col-md-6 col-sm-6 col-xs-12">
</div>
