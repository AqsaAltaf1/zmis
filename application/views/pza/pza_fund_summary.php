...
<h4 class="m-0 text-dark">Provincial Zakat Administration (PZA) Fund Allocation/Disbursement Summary</h4>
</div><!-- /.col -->
...
<?php
$runpza_query_amount = $this->db->select_sum('pza_amount')->from('pza_fund')->where('status',1)->where('financial_year',$getfinancial_year)->where('installment',$get_inst)->get();
//$this->db->last_query();
$total_pzareceived_amount = $runpza_query_amount->row('pza_amount');
echo number_format($runpza_query_amount->row('pza_amount'),2);
?>
...
<?php
$runpzf_query_hoa = $this->db->select_sum('amount_allocated')->from('head_wise_fund')->where('status',1)->where('financial_year',$getfinancial_year)->where('installment',$get_inst)->get();
