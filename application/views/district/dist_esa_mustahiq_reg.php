...
$esa_expense_query = $this->db->select_sum('amount')->from('mustahiqeen')
->where('district_id',$userid)->where('MustahiqType',"Educational Stipend (A)")->where('installment',$get_inst)->where('year',$getfinancial_year)->get();
$total_esa_expense = $esa_expense_query->row('amount');
...
<span class="info-box-number">Total ESA Fund</span>
<span class="info-box-number" style="color: blue;">
<?php
echo number_format($total_esa_amount,2);
?>
 <br>
</span>
...
<?php
echo number_format($total_esa_expense,2);
?>
<br>
</span>
...
<?php
$esa_balance = $total_esa_amount - $total_esa_expense;
echo number_format($esa_balance,2);
...
<h3 class="text-dark">Educational Stipend (Academic) Registration Form (LZ-19)</h3>
</div>
...
<h3 class="card-title">Educational Stipend (Academic) Mustahiq Entry Form</h3>
...
<div class="card-body">
<div class="row"></div>
<form id="" action="<?php echo base_url(); ?>Dist_ESA_mustahiq_reg/manage_esa_mustahiq_reg/" method="post" data-parsley-validate="" class="" enctype="multipart/form-data">
...
<input type="number" name="amount" id="amount" value="" class="form-control col-md-4 col-xs-12" placeholder="Amount" min="1" max="<?php echo $esa_balance;?>"  required>
...
<div class="card col-12 col-md-12 col-sm-6">
<div class="card-header">
<h1 class="card-title" style="font-size: 23px;">Educational Stipend (Academic) Students List of District <strong><?php echo $district_name; ?></strong> for Year:<strong><?php echo $getfinancial_year; ?></strong> and Inst:<strong><?php echo $get_inst; ?></strong></h1>
 <!-- Print list -->
<a target="_blank" href="<?php echo base_url(); ?>Dist_ESA_mustahiq_reg/EduStipendPrint" class="btn btn-primary btn-sm float-right">Print Edu Scholarship Mustahiqeen</a>
</div>
<!-- /.card-header -->
<div class="card-body">
...
<?php
$i=1;
if(!empty($dist_esa_mustahiq_list))
{
foreach($dist_esa_mustahiq_list as $dist_esa_mustahiq_listvalues)
{
?>
<tr>
<td><?php echo $i;?></td>
...
echo $dist_esa_mustahiq_listvalues['InstituteName'];
?>
</td>
<td><?php echo $dist_esa_mustahiq_listvalues['mustahiq_name']; ?></td>
<td><?php echo $dist_esa_mustahiq_listvalues['father_name']; ?></td>
<td><?php echo $dist_esa_mustahiq_listvalues['mustahiq_cnic']; ?></td>
<td><?php echo $dist_esa_mustahiq_listvalues['contact_number']; ?></td>
<td><?php echo $dist_esa_mustahiq_listvalues['LZCActualName'];?></td>
<td><?php echo $dist_esa_mustahiq_listvalues['Qualification'];?></td>
<td><?php echo $dist_esa_mustahiq_listvalues['stipendDuration'];?></td>
<td><?php echo $dist_esa_mustahiq_listvalues['amount']; ?></td>
<td><a  href="<?php echo base_url();?>Dist_ESA_mustahiq_reg/deleteESARecord/<?php echo $dist_esa_mustahiq_listvalues['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></td>
</tr>
<?php 
$i++;
}
