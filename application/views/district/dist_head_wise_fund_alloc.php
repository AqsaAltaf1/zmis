...
<select onChange="getTargetMustahiq(this.value);" class="form-control col-md-8" id="lzc" name="lzc">
...
{
?>
<option value="<?php echo $get_all_deeni_madarisvalue['madrassa_name']?>"><?php echo $get_all_deeni_madarisvalue['madrassa_name']; ?></option>
<?php 
$i++;
}
}
...
{
?>
<option value="Qazi Husain Ahmad Hospital">Qazi Husain Ahmad Hospital</option>
<?php 
}
?>
...
<input type="text" id="nobvalues" name="nob_ga" onKeyUp="calculatenobs_ga(this.value)" max="70" maxlength="2" min="1" class="form-control col-md-8 col-xs-12" readonly>
</div>

<div class="row form-group">
<label class="control-label col-md-3" for="mustahiq_no">Minimum No of Applicants Required <span class="required">*</span> </label>
<input type="text" id="nobvalues" name="nob_ga" onKeyUp="calculatenobs_ga(this.value)" max="70" maxlength="2" min="1" class="form-control col-md-8 col-xs-12" readonly>
</div>

<div class="row form-group">
<label class="control-label col-md-3" for="amount_alloc">Allocate Amount <span class="required">*</span> </label>
<input type="number" id="amount_allocatedvalue_ga" name="amount_allocated_ga" min="1" max="<?php echo $net_balance;?>" class="form-control col-md-8 col-xs-12" readonly>
</div>
</div>
...
->where('lzc_id',$LZC_name)->where('district_id',$userid)->where('payment_amount',12000)
->get();
...
<form id="pzf_fund" action="<?php echo base_url(); ?>Dist_head_wise_fund_alloc/chequeCancel/" method="post" data-parsley-validate="" class="" enctype="multipart/form-data">
...
<div class="row form-group">
<label class="control-label col-md-3" for="">Comments<span class="required">*</span> </label>
...
<div class="modal fade bs-example-modal-lg_1122<?php echo $i1;?>" tabindex="-1" role="dialog" style="display: none;" aria-hidden="true">
<div class="modal-dialog modal-lg">
<div class="modal-content">

<div class="modal-header pull-left">
<h4 class="modal-title" id="myModalLabel">Update District <strong><?php echo $district_name;?></strong> Guzara Allowance Fund Allocation</h4>
<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span>
</button>
...
$getdeenimadarisqry = $this->db->select('*')->from('madrassa_list')->where('id',$lzc_institution_madrasaid)->where('district_id',$district_id)->get();
 $getMadrassaName = $getdeenimadarisqry->row('madrassa_name');*/
  echo $getdeeni_madaras['lzc_institution_madrasa'];
?>
...
<td><?php echo $getdeeni_madaras['cheque_no'];?></td>
...
<td><?php echo $get_edulist['cheque_no'];?></td>
...
<td><?php echo $get_hc_listings_values['cheque_no'];?></td>
...
function getTargetMustahiq(lzcID)
{
$.ajax({
url:"<?php echo base_url(); ?>Dist_head_wise_fund_alloc/getTargetMustahiqeen",
method:"POST",
beforeSend: function() 
{
...
var totalvalue = parseInt(data);

var halfvalue = parseInt(totalvalue/2);



$('#nobvalues').val(halfvalue);
$('#amount_allocatedvalue_ga').val(halfvalue*12000);
