...
$hcOtherDHQ = $this->db->select_sum('amount_allocated')->from('dist_head_wise_fund')
->where('district_id',$userid)
->where('account_head','Health Care (District Level)')
->where('lzc_institution_madrasa','Qazi Husain Ahmad Hospital')
->where('installment',$get_inst)
->where('year',$getfinancial_year)
->get();

$totalOtherDHQ = $hcOtherDHQ->row('amount_allocated');
...
$otherDHQDisbursementQuery = $this->db->select_sum('amount')->from('hc_mustahiqeen')
->where('district_id',$userid)->where('dhq_id',$userid)->where('hospital_type','Qazi Husain Ahmad Hospital')->where('installment',$get_inst)->where('year',$getfinancial_year)->get();
$totalOtherDHQDisbursement = $otherDHQDisbursementQuery->row('amount');

$balanceOtherDHQ = $totalOtherDHQ - $totalOtherDHQDisbursement;
...
{
?>
<option value="Qazi Husain Ahmad Hospital.<?php echo $userid;?>">Qazi Husain Ahmad Hospital</option>
<?php 
}
?>
...
<div class="row form-group" style="display: none;" id="otherDHQ_div">
<label class="col-md-2" for="amount">Amount<span class="required">*</span>
</label>
<input type="number" name="otherDHQAmount" id="amount" value="" class="form-control col-md-4 col-xs-12" placeholder="Patient's Expense Amount" min="1" max="<?php echo $balanceOtherDHQ;?>">

<label class="col-md-2" for="">Other District Hospital Budget<span class="required">*</span>
</label>
<input type="number" name="budget" id="budget" value="<?php echo $balanceOtherDHQ;?>" class="form-control col-md-4 col-xs-12" readonly>
...
if(!empty($patientDHQ))
{
foreach($patientDHQ as $patientList)
{
?>
<tr>
<td><?php echo $i;?></td>
<td><?php echo $patientList['mustahiq_name']; ?></td>
<td><?php echo $patientList['f_name']; ?></td>
<td><?php echo $patientList['cnic']; ?></td>
<td><?php echo $patientList['istehqaq_no']; ?></td>
<td><?php echo $patientList['cell_no'];?></td>
<td><?php echo $patientList['LZCActualName']; ?></td>
<td><?php echo $patientList['hospitalName']; ?></td>
<td><?php echo $patientList['disease_type']; ?></td>
<td><?php echo $patientList['amount']; ?></td>
<td>

<a href="<?php echo base_url(); ?>Dist_HC_mustahiq_reg/updatePatient/<?php echo $patientList['id'];?>" class="btn btn-success btn-sm" target="_blank"> <i class="fas fa-edit"></i>Update</a>
...
else if ($(this).val()==="Qazi Husain Ahmad Hospital.<?php echo $userid;?>")
{
$("#otherDHQ_div").slideDown();
$("#hospital_div").slideUp(); 
$("#pt_expense_div").slideUp();
$("#thq_div").slideUp();
$("#bhu_div").slideUp();
