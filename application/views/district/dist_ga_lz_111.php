...
<h3 class="m-0 text-dark">Approval Summary of Most Vlnerable Mustahiqeen Regarding Guzara Allowance in respect of District_____</h3>
</div><!-- /.col -->
...
<span class="info-box-number">Registered NOBs</span>
...
<?php
$allocated_nobs_query = $this->db->select_sum('nob')->from('dist_head_wise_fund')->where('district_id',$userid)->where('account_head','Guzara Allowance')->where('status',1)->get();
...
<td>
<?php
$getfinancialdata = $this->db->select('*')->from('dist_head_wise_fund')->where('lzc_institution_madrasa',$lzcid)->where('account_head','Guzara Allowance')->where('year',$getfinancial_year)->where('installment',$get_inst)->get();
$get_nob = $getfinancialdata->row('nob');
if(($get_nob == "") || ($get_nob == ""))
{
	echo "0";	
}
...
<td><?php echo $targeted_nob = $get_nob * 5;?></td>
...
<?php
if(($get_nob == "") || ($get_nob == 0) || ($result <  $targeted_nob))
{
	?>
...
{
?>
<a href="<?php echo base_url(); ?>Dist_GA_Mustahiqeenlist/getmustahiqeenlist/<?php echo $lzcid; ?>" class="btn btn-success btn-sm"> View List</a></td>
<?php
}
?>
...
<p style="font-size:24px; letter-spacing:2px; text-align:center;">
<?php echo $links; ?>
</p>
...
Showing Records : <?php echo $totalcount;?>
