...
class Dist_ESP_mustahiq_reg extends CI_Controller {
	...
	{
	parent::__construct();
	$this->load->model('Dist_ESP_mustahiq_reg_model');
	...
	$getfinancial_installment = $getfinancialdata->row('installment');
		...
		$data['get_all_coleges_uni_list'] = $this->Dist_ESP_mustahiq_reg_model->get_all_coleges_uni_list($userid);
		...
		$data['get_esp_mustahiq_list'] = $this->Dist_ESP_mustahiq_reg_model->get_esp_mustahiq_list($userid,$getfinancial_year,$getfinancial_installment);
		
		
		$this->load->view('district/dist_esp_mustahiq_reg',$data);
	}
	
	
	
	
	function manage_esp_mustahiq_reg()
	{
	...
	$formArray = array();
	$formArray['MustahiqType'] = "Educational Stipend (P)";
	...
	redirect(base_url('Dist_ESP_mustahiq_reg'));
	exit;
	}
	...
	$this->Dist_ESP_mustahiq_reg_model->manage_esp_mustahiq_reg($formArray);		
	...
	$data['getEduStipendPrint'] = $this->Dist_ESP_mustahiq_reg_model->getEduStipendPrint($userid,$getfinancial_year,$getfinancial_installment);
	$this->load->view('district/distEduStipendPrintP',$data);
}
	
	
	function deleteESPRecord(){
		
		$studentID=$this->uri->segment(3);
	  	$response=$this->Dist_ESP_mustahiq_reg_model->deleteESPrecord($studentID);
	if($response==true){
