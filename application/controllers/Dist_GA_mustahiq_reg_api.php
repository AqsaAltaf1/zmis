...
function Login()
{
		...
		$chk_account = $this->User_login_model->chk_account($username,$password, 'gs',$version);
		...
		$data['get_all_lzc'] 		 = $this->Dist_login_model->get_all_lzc($userid,$district_id);
		$data['get_all_lzc_GA']		 = $this->Dist_login_model->get_all_lzc_GA($username);
		$data['get_all_lzc_LZ11'] 	 = $this->Dist_login_model->get_all_lzc_LZ11($username);
		$data['get_all_lzc_LZ19']    = $this->Dist_login_model->get_all_lzc_LZ19($username);
		...
		$myResponse->gs_lzc_name_GA=$data['get_all_lzc_GA'];
		$myResponse->gs_lzc_name_LZ19=$data['get_all_lzc_LZ19'];
		$myResponse->gs_lzc_name_LZ11=$data['get_all_lzc_LZ11']; 
		$myResponse->Enable_FormHawalgi="0";
		$myResponse->Enable_GAForm="0";
		$myResponse->Enable_ShowPayment="0";
	
		}
		else if($chk_account == "2"){
		$myResponse->Result = "Failed";
		$myResponse->Description = "براہ کرم اپنی ایپ کو تازہ  ورژن 2.4 میں اپ ڈیٹ کریں۔ مزید معلومات کے لیے واٹس ایپ گروپ سے رابطہ کریں۔";
...
function ZakatEntryForm()   //Save Zakat Entry Forms Data
{
		
		$LZC_Name  = $this->input->post("LZC_Name");
		$get_lzc_nameqry = $this->db->select('*')->from('lzc_list')->where('lzc_name',$LZC_Name)->get();
		
		$LZC_ID  		 = 		$get_lzc_nameqry->row('id');
		$ChairmanCNIC 	 = 		$this->input->post("ChairmanCNIC"); 
		$LZCName   	 	 = 		$this->input->post("LZC_Name"); 
		
		
		$chk_duplicate = $this->User_login_model->chk_duplicateChairman($LZC_ID,$ChairmanCNIC, $LZCName);
		
		
		/* $myfile = fopen("ZakatEntryForm.txt", "a") or die("Unable to open file!");
		$output =  $LZC_ID."<br />".  $ChairmanCNIC ."<br />".   $LZCName ."<br />". $chk_duplicate ."<br />";   //print_r($_POST, true);   //.print_r($_POST)
		...
		$myResponse->Description = "Data for this Chairman already exists - اس چیئرمین کا ڈیٹا پہلے سے موجود ہے۔";
		...
		$formArray['LZC_Code'] =  $get_lzc_nameqry->row('lzc_code');    		
		$formArray['LZC_Name'] =  $this->input->post("LZC_Name");     		
		...
		$myResponse->Result = "Successful";
		$myResponse->Description = "Record for LZC : ".  $this->input->post("LZC_Name") . "(" . $get_lzc_nameqry->row('lzc_code')  .") Added Successfully";
