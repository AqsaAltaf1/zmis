...
class AffidavitManagement extends CI_Controller 
{

function __construct()
{
parent::__construct();
$this->load->model('AffidavitManagement_model');
...
$data['getFormReceivedLZC'] = $this->AffidavitManagement_model->getFormReceivedLZC();

$data['get_districts'] = $this->General_model->get_districts();	
$this->load->view('pza/affidavitManagement',$data);
}
...
function updateZakatEntryForm()
	{
		...
		$this->load->view('pza/updateFormHandingOver',$data);
}


//Edit Zakat Affidavit

function editAffidavitForm()
	{
	

		$affidavitID  = $this->input->post("affidavitID");
		...
		//Affidavit Uploading
		$formPath = $affidavit['upload_path'] = '/zakatentryform/'; 
		...
		$formArray['Picture3']  = $formPath.$affidavitName['file_name'];  
		
		$date = date('Y-m-d H:i:s');
		$formArray['updatedOn'] = $date;
		$formArray['updatedBy'] = "PZA";
		$this->AffidavitManagement_model->editAffidavitForm($formArray,$affidavitID);

		$this->session->set_flashdata('success','Record Updated Successfully..!');
		
		redirect(base_url('AffidavitManagement'));
}
