...
class Ncf_allocation extends CI_Controller {
...
$this->load->model('Ncf_alllocation_model');
...
$formArray['status'] = 1;
	...
	$this->db->select("*");
	$this->db->from("natural_calamity_fund");
	...
	if($query->num_rows()>0)
	{
	$this->session->set_flashdata('error','Selected district already exists.');
	redirect(base_url('Pza_District_shares'));
	}
	else
	{
	$this->Ncf_alllocation_model->manage_ncf_payment($formArray);
	$this->session->set_flashdata('success','Data for NCF Inserted Successfully.');
	redirect(base_url('Pza_District_shares'));
	}
